...
def _remove_untracked_rectangles(annotations: dict) -> tuple[dict, int, int]:
    shapes = annotations.get("shapes", []) or []
    # Remove shapes that are rectangle/bbox types from the untracked shapes list
    kept_shapes = [shape for shape in shapes if shape.get("type") not in ("rectangle", "bbox", "rect")]
...
def _confirm_if_no_tracks(task_id: int, track_count: int) -> bool:
    ...
    answer = input(
        f"Task {task_id}: continue anyway and delete untracked rectangle bboxes? [y/N]: "
    ).strip().lower()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Delete all untracked rectangle/bbox shapes while preserving tracks for a CVAT task"
    )
    ...
    updated, removed_count, total_shapes = _remove_untracked_rectangles(annotations)

    print(f"Task {task_id}")
    print(f"  total shapes: {total_shapes}")
    print(f"  untracked rectangle/bbox shapes to remove: {removed_count}")
    ...
    print("Update complete. Untracked rectangle/bbox shapes removed and tracks preserved.")
